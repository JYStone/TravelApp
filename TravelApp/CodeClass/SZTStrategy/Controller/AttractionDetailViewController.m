...
- (void)viewDidLoad {
    ...
    NSString *collectionButtonName = nil;
    if ([[TravelFMDataBase shareTravelDataBase] selectRowWithTravelName:self.touristAttraction.name tableName:@"TouristAttractionDeail"]) {
         collectionButtonName = @"yw_detail_collectedicon@3x";
        
    }else{
        collectionButtonName = @"yw_detail_collecticon@3x";

    }
    
    UIBarButtonItem *collectionButton = [[UIBarButtonItem alloc]initWithImage:[[UIImage imageNamed:collectionButtonName]imageWithRenderingMode:(UIImageRenderingModeAlwaysOriginal)] style:(UIBarButtonItemStylePlain) target:self action:@selector(collectionAction:)];
...
-(UITableView *)tableView{
    if (!_tableView) {
        _tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, kIconImageY, kWidth, kHeight) style:(UITableViewStylePlain)];
...
-(NSMutableDictionary *)messageDict
{
    if (!_messageDict) {
        ...
        if (self.touristAttraction.tips.count == 0) {
            [LORequestManger GET:[NSString stringWithFormat:@"http://api.breadtrip.com/destination/place/%@/%@/",self.touristAttraction.type,self.touristAttraction.ID] success:^(id response) {
